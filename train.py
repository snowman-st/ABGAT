...
from models import LSTM, ASCNN, ASGCN,ASGAT,ASGAT2,ASGAT3
...
class Instructor:
    ...
    def run(self, repeats=2):
        ...
        if not os.path.exists(self.opt.logfile):
            os.mkdir(self.opt.logfile)

        f_out = open(self.opt.logfile+self.opt.model_name+'_'+self.opt.dataset+'_val.txt', 'a+', encoding='utf-8')
        ...
        for i in range(repeats):
            print('repeat: ', (i+1))
            f_out.write('repeat: {}\n'.format(i+1))
            f_out.write('optimizer:{}\tlearning_rate:{}\thhead:{}\tdropout:{}\n'.format(self.opt.optimizer,self.opt.learning_rate,self.opt.nheads,self.opt.dropout))
            self._reset_params()
            ...
            f_out.write('max_test_acc: {:.5f}\t\t max_test_f1: {:.5f}\n'.format(max_test_acc, max_test_f1))
...
if __name__ == '__main__':
    ...
    opt.device = torch.device('cuda:{}'.format(opt.device))
